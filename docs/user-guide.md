...
- **Your admin may skip that confirmation entirely.** If they've turned on zero-touch setup for a package type, the plugin binds it to Artifactory automatically in the background the first time you start a session, before you even ask for anything. You won't see a prompt at all for that package type; it just works from the first request.
