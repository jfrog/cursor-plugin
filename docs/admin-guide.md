...
| Force a refresh of the cached repo snapshot            | Set `"cacheTtlDays": 0`, or edit `agents-conf.json` (cache invalidates on file change) |
...
Package types not listed in `defaultGlobalRepos` remain unresolved for that session until the user runs setup (the agent will offer to do this for them) or you add the mapping.
...
Other package types stay unresolved until you expand the map.
